...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate input
    if (empty($fullname) || empty($adminname) || empty($telepon) || empty($email) || empty($password)) {
        echo "Semua field harus diisi!";
        exit;
    }

    // Prepare SQL statement to prevent SQL injection
    $query_sql = $conn->prepare("INSERT INTO tb_loginadmin (fullname, adminname, telepon, email, password) VALUES (?, ?, ?, ?, ?)");
    $query_sql->bind_param("sssss", $fullname, $adminname, $telepon, $email, $password);
...
<body class="min-h-screen flex justify-center items-center" style="background-image: linear-gradient(225deg, #87c7ff 0, #028fe3 50%, #005cba 100%);">
    <div class="container">
            ...
            <div class="w-full md:w-1/2 lg:w-1/3">
                <div class="bg-white card rounded-lg shadow-md animate__animated animate__fadeInDown">
                    <div class="text-white rounded-t-lg py-4 text-center" style="background-color: #fda085;">
                        <h3><i class="fas fa-user-plus"></i> Ayo Register Sebagai Admin !</h3>
                    </div>
                    <div class="px-6 py-8">
